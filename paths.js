...
module.exports = {
  ...
  contexts: [
    {
      path: path.dirname(
        getSubmodulePath("vscode/src/vs/base/worker/workerMain.ts")
      ),
      includes: {
        "vs/base/common/worker/simpleWorker": getSubmodulePath(
          "vscode/src/vs/base/common/worker/simpleWorker.ts"
        )
      }
    },
    {
      path: path.dirname(
        getSubmodulePath("vscode/src/vs/base/common/worker/simpleWorker.ts")
      ),
      includes: {
        "vs/editor/common/services/editorSimpleWorker": getSubmodulePath(
          "vscode/src/vs/editor/common/services/editorSimpleWorker.ts"
        )
      }
    },
    {
      ...
      includes: {
        "vs/language/css/cssWorker": getSubmodulePath(
          "monaco-css/src/cssWorker.ts"
        ),
        "vs/language/html/htmlWorker": getSubmodulePath(
          "monaco-html/src/htmlWorker.ts"
        ),
        "vs/language/json/jsonWorker": getSubmodulePath(
          "monaco-json/src/jsonWorker.ts"
        ),
        "vs/language/typescript/src/worker": getSubmodulePath(
    ...
    {
      path: path.dirname(
        getSubmodulePath("monaco-languages/src/monaco.contribution.ts")
      ),
      includes: fs.readdirSync(getSubmodulePath("monaco-languages/src")).reduce(
        (acc, filename) =>
          Object.assign(acc, {
            [`./${path.basename(filename, path.extname(filename))}`]: path.join(
              getSubmodulePath("monaco-languages/src"),
              filename
            )
          }),
        {}
  ...
  umd: [getSubmodulePath("vscode-css-languageservice/src/data/browsers.js")]
};
